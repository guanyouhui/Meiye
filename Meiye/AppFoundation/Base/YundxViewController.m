...
@interface YundxViewController ()<MBProgressHUDDelegate> {
...
@implementation YundxViewController
...
- (void)showNetworkErrorView {
    ...
    [JoProgressHUD makeToast:@"网络不给力哦！"];
...
- (void)shareWithUrl:(NSString*)url content:(NSString*)content image:(UIImage*)image delegate:(id<UMSocialUIDelegate>)delegate {
    //
    //    NSString * shareUrl=@"http://www.paixie.net/";
    //    UIImage * shareImage=[UIImage imageNamed:@"Icon"];
    //    NSString *statusString = [UIUtils getShareContent:shareUrl content:@"测试内容" name:@"名字" platform:DEVICE_IPHONE];
    //
    [UMSocialData defaultData].shareText =content;
    [UMSocialData defaultData].shareImage =image;
    
    if ([ZUtilsString isNotEmpty:url]) {
    ...
    [UMSocialSnsService presentSnsIconSheetView:(UIViewController*)delegate appKey:UMENG_APPKEY shareText:content shareImage:image shareToSnsNames:@[ UMShareToWechatSession,UMShareToWechatTimeline, UMShareToSina,UMShareToQQ, UMShareToQzone] delegate:delegate];
