...
- (void)_init
{
    ...
    NSArray *pageControlVConstraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:[pageControl]-(-10)-|"
                                                                               options:kNilOptions
                                                                               metrics:nil
...
- (void)initWithCount:(NSInteger)count delegate:(id<ImagePlayerViewDelegate>)delegate edgeInsets:(UIEdgeInsets)edgeInsets
{
    ...
    for (int i = 0; i < count; i++) {
        startX = i * width;
        EGOImageView *imageView = [[EGOImageView alloc] initWithFrame:CGRectMake(startX, 0, width, height)];
        if ([[self.imageURLs firstObject] isKindOfClass:[NSURL class]]) {
            imageView.imageURL = self.imageURLs[i];
        }else if ([[self.imageURLs firstObject] isKindOfClass:[NSString class]]) {
            imageView.imageURL = [NSURL URLWithString:self.imageURLs[i]];
        }
        ...
        if ([self.imagePlayerViewDelegate respondsToSelector:@selector(imagePlayerView:loadImageForImageView:index:)]) {
