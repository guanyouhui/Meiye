...
@implementation JoProgressHUD

#pragma mark - Toast Methods

+ (void)makeToast:(NSString *)message {
//    [JoProgressHUD makeToast:message duration:CSToastDefaultDuration position:CSToastDefaultPosition];
    [JoProgressHUD makeToast:message duration:[self displayDurationForString:message] position:CSToastDefaultPosition];
}
...
+ (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position {
    UIView *toast = [JoProgressHUD viewForMessage:message title:nil image:nil];
    [JoProgressHUD showToast:toast duration:interval position:position];
}

+ (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position title:(NSString *)title {
    UIView *toast = [JoProgressHUD viewForMessage:message title:title image:nil];
    [JoProgressHUD showToast:toast duration:interval position:position];
}

+ (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position image:(UIImage *)image {
    UIView *toast = [JoProgressHUD viewForMessage:message title:nil image:image];
    [JoProgressHUD showToast:toast duration:interval position:position];
}

+ (void)makeToast:(NSString *)message duration:(CGFloat)interval  position:(id)position title:(NSString *)title image:(UIImage *)image {
    UIView *toast = [JoProgressHUD viewForMessage:message title:title image:image];
    [JoProgressHUD showToast:toast duration:interval position:position];
}

+ (void)showToast:(UIView *)toast {
    [JoProgressHUD showToast:toast duration:CSToastDefaultDuration position:CSToastDefaultPosition];
}

+ (void)showToast:(UIView *)toast duration:(CGFloat)interval position:(id)point {
    toast.center = [JoProgressHUD centerPointForPosition:point withToast:toast];
    toast.alpha = 0.0;
    ...
    UIWindow * keyWindow = (UIWindow *)[windows lastObject];
...
+ (void)makeToastActivity {
    [JoProgressHUD makeToastActivity:CSToastActivityDefaultPosition];
}
...
+ (void)makeToastActivity:(id)position {
    ...
    activityView.center = [JoProgressHUD centerPointForPosition:position withToast:activityView];
    ...
    //添加覆盖层（不可点击）
    UIView * activityBGView=[[UIView alloc]initWithFrame:[JoProgressHUD topWindow].frame];
    ...
    [[JoProgressHUD topWindow] addSubview:activityBGView];
...
+ (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            return CGPointMake([JoProgressHUD topWindow].bounds.size.width/2, (toast.frame.size.height / 2) + CSToastTopPadding);
        } else if([point caseInsensitiveCompare:@"bottom"] == NSOrderedSame) {
            return CGPointMake([JoProgressHUD topWindow].bounds.size.width/2, ([JoProgressHUD topWindow].bounds.size.height - (toast.frame.size.height / 2)) - CSToastBottomPadding);
        } else if([point caseInsensitiveCompare:@"center"] == NSOrderedSame) {
            return CGPointMake([JoProgressHUD topWindow].bounds.size.width / 2, [JoProgressHUD topWindow].bounds.size.height  / 2 );
        }
    ...
    return [JoProgressHUD centerPointForPosition:CSToastDefaultPosition withToast:toast];
}
...
+ (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (title != nil) {
        ...
        CGSize maxSizeTitle = CGSizeMake(([JoProgressHUD topWindow].bounds.size.width * CSToastMaxWidth) - imageWidth, [JoProgressHUD topWindow].bounds.size.height * CSToastMaxHeight);
    ...
    if (message != nil) {
        ...
        CGSize maxSizeMessage = CGSizeMake(([JoProgressHUD topWindow].bounds.size.width * CSToastMaxWidth) - imageWidth, [JoProgressHUD topWindow].bounds.size.height * CSToastMaxHeight);
