...
@interface TXImageCropperViewController ()
...
@implementation TXImageCropperViewController
...
- (id)initWithImage:(UIImage *)originalImage cropSize:(CGSize)cropSize limitScaleRatio:(NSInteger)limitRatio {
    ...
    if (self) {
        ...
        CGFloat txImageHigth=ScreenBounds.size.width/cropSize.width*cropSize.height;
        CGFloat txImageY=(ScreenBounds.size.height-txImageHigth)/2;
        self.cropFrame =CGRectMake(0, txImageY, ScreenBounds.size.width, txImageHigth);
...
- (void)initView {
    self.showImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 200, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
...
- (void)initControlBtn {
    ...
    bgView.backgroundColor=[UIColor colorWithRed:0/255.0f green:0/255.0f blue:0/255.0f alpha:0.5];
    [self.view addSubview:bgView];
    
    UIButton *cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 100, 50)];
    cancelBtn.backgroundColor = [UIColor clearColor];
    cancelBtn.titleLabel.textColor = [UIColor whiteColor];
    [cancelBtn setTitle:@"重新" forState:UIControlStateNormal];
...
- (void)cancel:(id)sender {
    [self back];
//    if (self.delegate && [self.delegate conformsToProtocol:@protocol(TXImageCropperDelegate)]) {
...
- (void)confirm:(id)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(imageCropper:didFinished:)]) {
        [self.delegate imageCropper:self didFinished:[self imageWithImageSimple:[self getSubImage] scaledToSize:self.cropSize]];
    }
}
...
//    //拍照的 要旋转 90°
//    if (self.o_entryType == UIImagePickerControllerSourceTypeCamera) {
