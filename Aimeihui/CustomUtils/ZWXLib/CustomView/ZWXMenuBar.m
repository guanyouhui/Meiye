...
#define  TitleNeedHeight 55
#define  CancleNeedHeight 65
//固定的 【行】 间距 （上下）
const CGFloat  kItemRowSpace = 10.0;
//上下 左右边距设置
const CGFloat kPanelViewTopMargin = 0.0f;
const CGFloat kPanelViewBottomMargin = 30.0f;
...
-(void)layoutSubviews
{
    ...
    [_label setFrame:CGRectMake(-5, self.frame.size.width+3, self.frame.size.width+10, 20)];
}
...
-(void) initSubView
{
    ...
    for (NSInteger page = 0; page<_pages; page++) {
        ...
        for (NSInteger index=page*(_realRows*_realColoumns); index<(page+1)*(_realRows*_realColoumns); index++) {
            ...
            CGFloat y = kPanelViewTopMargin + ((ItemSize + 20 +_realRowSpace)*row);
            
            item.frame = CGRectMake(x, y, ItemSize, ItemSize+20.0);
...
{
    return kPanelViewTopMargin + kPanelViewBottomMargin + _realRowSpace*(_realRows -1) + (ItemSize+20)*_realRows;
}
...
-(void) addMaskView
{
    ...
    [baseView addTarget:self action:@selector(dismiss) forControlEvents:UIControlEventTouchUpInside];
...
- (void) panelAddCancelButton
{
    ...
    _cancelButton.frame = CGRectMake(25, CGRectGetMaxY(_scrollView.frame) , 270, 40);
